...
public class PartButtonPhase : MonoBehaviour
{
    ...
    {
        part = p; phase = phaseCtrl; slot = s;

        if (label) label.text = string.IsNullOrEmpty(p.displayName) ? p.name : p.displayName;
        if (icon) { icon.sprite = p.icon; icon.enabled = (p.icon != null); }
    ...
    public void SetSelected(bool on)
    {
        ...
        var c = b.colors;
        c.normalColor = on ? new Color(0.85f, 0.9f, 1f) : Color.white;
        c.highlightedColor = on ? new Color(0.9f, 0.95f, 1f) : new Color(0.95f, 0.95f, 0.95f);
        b.colors = c;
    }
}
